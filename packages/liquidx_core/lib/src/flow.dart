...
typedef ComputeFlow<T> = T Function();
...
class Flow<T> implements ReadDrop<T>, ReactiveSource {
  Flow(this._compute, {String? label}) : label = label ?? 'flow<$T>';

  final ComputeFlow<T> _compute;
  @override
  final String label;
  ...
  void _recompute() {
    ...
    final Set<ReactiveSource> toRemove =
        _dependencies.difference(nextDependencies);
    final Set<ReactiveSource> toAdd =
        nextDependencies.difference(_dependencies);
    ...
    LiquidDiagnostics.emit(
      LiquidEvent(
        type: LiquidEventType.flowRecomputed,
        label: label,
  ...
  void _assertNotDisposed() {
    if (_disposed) {
      throw StateError('Flow "$label" was already disposed.');
    }
  }
}
